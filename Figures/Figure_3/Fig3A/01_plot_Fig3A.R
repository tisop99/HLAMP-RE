...
# (de-)methylation tracks
IDs <- fread('samplesheet_blank.csv',header=T)
IDs_LUSC <- IDs[IDs$tissue=="LUSC",]
IDs_BRCA <- IDs[IDs$tissue=="BRCA",]

meth_track_list <- function(path) {
  dt <- readRDS(path)
  gTrack(dt2gr(dt[modified_base_code == 'm'& Nvalid_cov > 5]), y.field='neglog_mth', bars=T, y0=0,y1=3)
}

LUSC_meth_track_list <- lapply(IDs_LUSC$bedmethylPath, meth_track_list)
names(LUSC_meth_track_list) <- IDs_LUSC$sid
BRCA_meth_track_list <- lapply(IDs_BRCA$bedmethylPath, meth_track_list)
names(BRCA_meth_track_list) <- IDs_BRCA$sid
